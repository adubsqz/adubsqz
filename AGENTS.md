...
- Gallery manifest records finished work only (model B; no in-progress rows). Humans declare `bw` / `color` / `redscale` / `about` import buckets (legacy `still-life` maps to `about`), symlink vs copy, and optional `photo-prompt` text.
